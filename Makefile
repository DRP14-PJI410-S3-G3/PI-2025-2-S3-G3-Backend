...
PORT=80
IMAGE_NAME="univesp.pi20250-2-s3g3-backend"
IMAGE_BASE="univesp.pi20250-2-s3g3-backend-base-image"

PROJECT_NAME=univesp-420315
SERVICE_NAME=univesp-pi20250s2g3-backend
...
create-gcr: ##Create Google Cloud Run Service with new image
	gcloud config set project ${PROJECT_NAME}
	gcloud run deploy ${SERVICE_NAME} --image=gcr.io/${PROJECT_NAME}/${IMAGE_NAME}:latest --platform managed --region=${SERVICE_REGION} --allow-unauthenticated --cpu 1 --memory 512Mi --min-instances 0 --max-instances 2
